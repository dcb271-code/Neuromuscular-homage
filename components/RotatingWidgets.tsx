...
// Pick 3 items spread across the pool so they rotate with no same-day overlap
function pick3<T>(arr: T[]): [T, T, T] {
  const n = arr.length;
  const idx = todayIndex() % n;
  return [
    arr[idx],
    arr[(idx + Math.floor(n / 3)) % n],
    arr[(idx + Math.floor((2 * n) / 3)) % n],
  ];
}
...
const ROTATING_CATEGORIES: Category[] = [
  ...
  { name: 'DNA Repair Neuropathies',         color: '#7c3aed', url: 'https://neuromuscular.wustl.edu/mother/dnarep.htm' },
...
export function RotatingCategories() {
  ...
  useEffect(() => {
    setItems(pick3(ROTATING_CATEGORIES));
    setMounted(true);
  }, []);
...
const ROTATING_CONDITIONS: Condition[] = [
  ...
  { label: 'Santavuori-Haltia Disease (NCL1)',     abbr: 'NCL1',     gene: 'CLN1',     color: '#db2777', url: 'https://neuromuscular.wustl.edu/syncm.html#santavuori' },
  ...
  { label: 'C12orf65 Mitochondrial Disease',       abbr: 'C12orf65', gene: 'C12orf65', color: '#d97706', url: 'https://neuromuscular.wustl.edu/mitosyn.html#c12orf65' },
...
export function RotatingConditions() {
  ...
  useEffect(() => {
    setItems(pick3(ROTATING_CONDITIONS));
    setMounted(true);
  }, []);
